...
因此，任何与静态通量比例一致的稳态解都必须满足 $G_I(\bar n_I)\bar u_I=\Lambda_I$。本文进一步按定义 \ref{def:target_component} 预先选取内点控制 $\bar u_I=1-\eta_I$，再在自由流分支上取
\begin{equation}
...
\begin{remark}
关键区别：$G_I(\bar{n}_I)\bar u_I = \Lambda_I$ 不再是人为引入的假设，而是稳态方程组与静态规划一致性（通过 $\beta$ 和稳态比例条件 \ref{eq:steady_static_ratio} 保证）的\textbf{必然推论}。本文不再将目标控制取为 $\bar u_I=1$，而是保留 $\eta_I$ 的控制裕度，以保证局部线性化使用的是真实存在的光滑内点邻域。
\end{remark}
...
本节设计标量周界跟踪控制器，以第 5 节推导的流量层静态--动态桥接均衡点为目标状态。由于本文主模型已限定为无时延、无显式排队的理想宏观系统，稳定性分析直接针对该基础空域子系统展开。外部输入和路由比例在分析中固定；在目标点附近的自由流不饱和邻域内，本文给出单区域局部渐近稳定性证明和多区域组分线性化 Hurwitz 判据。
...
在多区域网络中，邻居流入不仅依赖区域总积累量 $e_I$，还依赖目的地组分比例 $\pi_I^D$。因此，仅用总量误差构造的 Lyapunov 函数不能单独控制组分比例扰动。本节改用静态可达支持集内的组分误差进行局部稳定性分析。
...
\end{equation}
以下线性化只在静态可达支持集 $\mathcal S$ 上构造；矩阵 $A$ 的行、列索引均为满足 $(I,D)\in\mathcal S$ 的组分，且 $e_I=\sum_{D:(I,D)\in\mathcal S}z_I^D$。对 $\mathcal S$ 外的组分，假设 \ref{asm:support_consistency} 要求初始状态和输入为零，因此不进入线性化状态向量。
...
\begin{enumerate}
    ...
    \end{equation}
\end{enumerate}
则静态可达支持集 $\mathcal S$ 上的组分误差 $\bm z_{\mathcal S}(t)$ 在该邻域内局部渐近收敛至零。因此对纳入 $\mathcal S$ 的区域--目的地组分，总量误差 $e_I(t)=\sum_{D:(I,D)\in\mathcal S} z_I^D(t)$ 也收敛至零。若扰动产生 $\mathcal S$ 外的非零组分，则需启用注 \ref{rem:fallback_routing} 的备用路由或在线重规划，并基于实际路由重新构造 $A$；本定理不覆盖该扩展系统。
\end{theorem}
...
\textbf{输入}：静态规划结果 $\{f_m^*, S_m^{(p)}, \alpha_m^{(p),*}\}$，可查询的标定 MFD 对象 $\{G_I(\cdot),G_{I,s}^{-1},G_I^{\max}, n_I^{cr,*}\}$，控制裕度 $\{\bar u_I\}_{I\in\mathcal R_+}$，控制增益 $\{\kappa_I\}_{I\in\mathcal R_+}$，正则化流率 $\{\epsilon_I\}_{I\in\mathcal R_+}$。
